...
run_cmd() {
  ...
  if [ $EXIT_CODE -ne 0 ]; then
    ...
    echo "Then try running 'npm run local:deploy' again."
    exit $EXIT_CODE
  fi
  echo "---------------------------------------------------------"
}

run_cmd "cfnless deploy" cfnless deploy
run_cmd "cfnless info"   cfnless info

echo "Local deploy ran successfully!"