...
// Only container functions are tested here. Zip functions require LocalStack to run the Node.js
// runtime inside Docker (Docker-in-Docker), which is not available in the current local-executor
// setup. The createOrUpdateZipFunction code path is covered by unit tests in lambda-manager.test.ts.
